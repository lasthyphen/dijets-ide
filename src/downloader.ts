/**
 * Binary downloader for the Dijets IDE.
 * 
...
export class Downloader {
    ...
    get zipPath(): string {
        return this.binPath('fetched.zip');
    }
...
async function checkNewRelease(this: Downloader): Promise<any> {
    const options = {
        host: 'api.github.com',
        path: `/repos/lasthyphen/dijets-vscode/releases/latest`,
        method: 'GET',
        headers: {'user-agent': 'node.js'}
    };
    ...
    // @ts-ignore 
    // Important! This line searches for a release with "dijets" and "<PLATFORM>" in its name
    const release = stats.assets.find((a) => a.name.includes(PLATFORM) && a.name.includes('dijets')) || null;
