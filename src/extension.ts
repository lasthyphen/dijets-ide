/**
 * Entry Point for the Dijets IDE Extension.
 *
 * @copyright 2021 Dijets.
 */
 ...
 */
const NAMESPACE = 'dijets';

/**
 * Name of the extension to get context.
 */
const EXTENSION = 'dijets.ide';
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    context.subscriptions.push(
        registerCommand('dijets.check', () => checkCommand().then(console.log)),
        registerCommand('dijets.clean', () => cleanCommand().then(console.log)),
        registerCommand('dijets.doctor', () => doctorCommand().then(console.log)),
        registerCommand('dijets.testFunctional', () => testFunctionalCommand().then(console.log)),
        registerCommand('dijets.publish', () => publishCommand().then(console.log)),
        registerCommand('dijets.run', () => runCommand().then(console.log)),
        registerCommand('dijets.testUnit', () => testUnitCommand().then(console.log)),
        registerCommand('dijets.publishAll', () => publishAllCommand().then(console.log)),
    );
}
...
function execute(task: string, command: string, fileMarker: Marker): Thenable<any> {
    ...
    const commonArgs: string[] = [
        ...
        ['--dijets-rpc', configuration.get<string>('dijetsRPC') || null],
